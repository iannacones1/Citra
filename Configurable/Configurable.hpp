...
#define CONFIG_EACH_OLD(r, data, x) Citra::Configurable::IConfigurator::Instance()->get(inClassName, XSTR(STRIP(x)), STRIP(x));
#define CONFIG_EACH(r, data, x) inConfiguratorPtr->get(inClassName, XSTR(STRIP(x)), STRIP(x));

#define CONFIGURABLE(...)                                                            \
BOOST_PP_SEQ_FOR_EACH_I(DECLARE_EACH, data, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__))   \
virtual void configure(Citra::Configurable::IConfigurator* inConfiguratorPtr, const std::string& inClassName)                               \
{                                                                                    \
...
{                                                 \
  ...
  {                                               \
    ...
    ptr->configure(Citra::Configurable::IConfigurator::Instance(), #className); \
  }                                               \
};                                                \
